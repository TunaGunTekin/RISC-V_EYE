...
module execute_stage_eye(
    ...
    input logic                       alu_src_select_in,
    input logic [3:0]                 alu_op_select_in,

    // YENİ EKLENEN KONTROL SİNYALLERİ (Decoder'dan gelmeli)
    ...
    always_comb begin   
        case (forward_a_select_in) //Burada hazard unit sinyallerine dikkat edilmeli
    ...
    // Bu Mux'un çıkışı (WriteDataE), hem ALU'ya hem de Store işlemi için belleğe gider.
    always_comb begin   
        case (forward_b_select_in) //Burada hazard unit sinyallerine dikkat edilmeli
    ...
    assign alu_src_b_final = (alu_src_select_in) ? imm_extended_in : forwarded_src_b;

    // --- 3. ALU Source A Selection (Reg vs PC) ---
    // AUIPC komutu için PC değerini ALU'ya sokmamız gerekir.
    // LUI komutu için genelde rs1=0 (x0) kullanılır, bu yüzden Reg seçimi yeterlidir.
    assign alu_src_a_final = (alu_src_a_select_in) ? pc_in : forwarded_src_a;
    ...
    alu_eye alu_inst (
        ...
        .ov_flag_out(ov_flag_out),
        .sign_flag_out(sign_flag_out)
    );
    
    assign branch_target_calc = pc_in + imm_extended_in;
